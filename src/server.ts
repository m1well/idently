...
/**
 * send code and audience in header and get long valid jwt token (if code is found)
 */
// deno-lint-ignore no-explicit-any
...
router.get('/token/verify', async (ctx: RouterContext<any>) => {
  const audience = ctx.request.headers.get(REQUIRED_SOURCE_HEADER);
  ...
  const response = await handleVerifyToken(ctx.request, audience);
  ctx.response.status = response.status;
  try {
    ctx.response.body = await response.json();
  } catch (_e) {
